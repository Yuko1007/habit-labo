...
export default function RegisterPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (data.success) {
        setSuccess('登録が完了しました。ログイン画面へリダイレクトします...');
        setTimeout(() => {
          router.push('/login');
        }, 2000);
      } else {
    ...
    <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: '#F8FAFC', padding: '3rem 1rem' }}>
      <div style={{ width: '100%', maxWidth: '28rem', display: 'flex', flexDirection: 'column', gap: '1.5rem' }}>
        ...
        <div style={{ textAlign: 'center' }}>
          ...
          <p style={{ color: '#6B7280', fontSize: '14px', borderTop: '1px solid #E5E7EB', paddingTop: '0.5rem', margin: 0 }}>ユーザー登録</p>
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1.5rem', backgroundColor: '#FFFFFF', padding: '1.5rem', borderRadius: '1rem', boxShadow: '0 1px 3px rgba(0,0,0,0.05)', border: '1px solid #E5E7EB' }}>
          ...
          {/* ニックネーム */}
          <div>
            <label htmlFor="nickname" style={{ display: 'block', fontSize: '12px', fontWeight: '600', color: '#6B7280', marginBottom: '0.5rem', textTransform: 'uppercase', letterSpacing: '0.05em' }}>
              ニックネーム
            </label>
            <input
              id="nickname"
          ...
          <div style={{ textAlign: 'center', fontSize: '12px', color: '#6B7280' }}>
            <p style={{ margin: 0 }}>
              すでにアカウントをお持ちの方は
              <Link href="/login" style={{ color: '#4F46E5', textDecoration: 'none', marginLeft: '0.25rem' }} onMouseEnter={(e) => { e.currentTarget.style.textDecoration = 'underline'; }} onMouseLeave={(e) => { e.currentTarget.style.textDecoration = 'none'; }}>
                ログイン
              </Link>
            </p>
          </div>
